    ...
    stage('commit') {
      parallel {
        stage('adventureCore: commit') {
          steps {
            dir(path: 'adventureCore') {
              git(url: 'git@github.com:Adventure-RPG/adventure-core.git', branch: 'master', credentialsId: 'adventure_main_rsa', changelog: true)
              sh 'echo $GIT_SSH'
        ...
        stage('adventureUAA: commit') {
          steps {
            dir(path: 'adventureUAA') {
              git(url: 'git@github.com:Adventure-RPG/adventure-uaa.git', branch: 'master', changelog: true, credentialsId: 'adventure_main_rsa')
              sh 'echo $GIT_SSH'
              sh 'git push origin master --force'
            }
