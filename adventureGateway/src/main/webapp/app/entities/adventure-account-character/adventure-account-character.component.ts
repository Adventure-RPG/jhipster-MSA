...
export class AdventureAccountCharacterComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.adventureAccountCharacterService
                ...
                .subscribe(
                    (res: HttpResponse<IAdventureAccountCharacter[]>) => this.paginateAdventureAccountCharacters(res.body, res.headers),
    ...
    trackId(index: number, item: IAdventureAccountCharacter) {
        return item.id;
    }

    registerChangeInAdventureAccountCharacters() {
        this.eventSubscriber = this.eventManager.subscribe('adventureAccountCharacterListModification', response => this.loadAll());
    }
    ...
    protected paginateAdventureAccountCharacters(data: IAdventureAccountCharacter[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.adventureAccountCharacters = data;
    }
