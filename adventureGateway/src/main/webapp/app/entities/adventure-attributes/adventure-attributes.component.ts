...
export class AdventureAttributesComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.adventureAttributesService
        ...
        .subscribe(
          (res: HttpResponse<IAdventureAttributes[]>) => this.paginateAdventureAttributes(res.body, res.headers),
  ...
  trackId(index: number, item: IAdventureAttributes) {
    return item.id;
  }

  registerChangeInAdventureAttributes() {
    this.eventSubscriber = this.eventManager.subscribe('adventureAttributesListModification', response => this.loadAll());
  }
  ...
  protected paginateAdventureAttributes(data: IAdventureAttributes[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.adventureAttributes = data;
  }
