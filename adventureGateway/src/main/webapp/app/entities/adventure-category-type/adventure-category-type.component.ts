...
export class AdventureCategoryTypeComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.adventureCategoryTypeService
        ...
        .subscribe(
          (res: HttpResponse<IAdventureCategoryType[]>) => this.paginateAdventureCategoryTypes(res.body, res.headers),
  ...
  trackId(index: number, item: IAdventureCategoryType) {
    return item.id;
  }

  registerChangeInAdventureCategoryTypes() {
    this.eventSubscriber = this.eventManager.subscribe('adventureCategoryTypeListModification', response => this.loadAll());
  }
  ...
  protected paginateAdventureCategoryTypes(data: IAdventureCategoryType[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.adventureCategoryTypes = data;
  }
