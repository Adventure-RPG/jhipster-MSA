...
export class AdventureCharacteristicComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.adventureCharacteristicService
                ...
                .subscribe(
                    (res: HttpResponse<IAdventureCharacteristic[]>) => this.paginateAdventureCharacteristics(res.body, res.headers),
    ...
    trackId(index: number, item: IAdventureCharacteristic) {
        return item.id;
    }

    registerChangeInAdventureCharacteristics() {
        this.eventSubscriber = this.eventManager.subscribe('adventureCharacteristicListModification', response => this.loadAll());
    }
    ...
    protected paginateAdventureCharacteristics(data: IAdventureCharacteristic[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.adventureCharacteristics = data;
    }
