...
export class AdventureInventoryCharComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.adventureInventoryCharService
        ...
        .subscribe(
          (res: HttpResponse<IAdventureInventoryChar[]>) => this.paginateAdventureInventoryChars(res.body, res.headers),
  ...
  trackId(index: number, item: IAdventureInventoryChar) {
    return item.id;
  }

  registerChangeInAdventureInventoryChars() {
    this.eventSubscriber = this.eventManager.subscribe('adventureInventoryCharListModification', response => this.loadAll());
  }
  ...
  protected paginateAdventureInventoryChars(data: IAdventureInventoryChar[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.adventureInventoryChars = data;
  }
