...
export class AdventureRaceOptionsComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        if (this.currentSearch) {
            this.adventureRaceOptionsService
                ...
                .subscribe(
                    (res: HttpResponse<IAdventureRaceOptions[]>) => this.paginateAdventureRaceOptions(res.body, res.headers),
    ...
    trackId(index: number, item: IAdventureRaceOptions) {
        return item.id;
    }

    registerChangeInAdventureRaceOptions() {
        this.eventSubscriber = this.eventManager.subscribe('adventureRaceOptionsListModification', response => this.loadAll());
    }
    ...
    protected paginateAdventureRaceOptions(data: IAdventureRaceOptions[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.adventureRaceOptions = data;
    }
