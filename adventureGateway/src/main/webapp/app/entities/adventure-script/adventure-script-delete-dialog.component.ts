...
export class AdventureScriptDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ adventureScript }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(AdventureScriptDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
        this.ngbModalRef.componentInstance.adventureScript = adventureScript;
        this.ngbModalRef.result.then(
          result => {
            this.router.navigate(['/adventure-script', { outlets: { popup: null } }]);
