...
export class AdventureSkillComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.adventureSkillService
        ...
        .subscribe(
          (res: HttpResponse<IAdventureSkill[]>) => this.paginateAdventureSkills(res.body, res.headers),
  ...
  trackId(index: number, item: IAdventureSkill) {
    return item.id;
  }

  registerChangeInAdventureSkills() {
    this.eventSubscriber = this.eventManager.subscribe('adventureSkillListModification', response => this.loadAll());
  }
  ...
  protected paginateAdventureSkills(data: IAdventureSkill[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.adventureSkills = data;
  }
