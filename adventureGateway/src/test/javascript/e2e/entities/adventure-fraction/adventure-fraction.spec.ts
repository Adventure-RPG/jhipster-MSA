...
describe('AdventureFraction e2e test', () => {
    ...
    it('should create and save AdventureFractions', async () => {
        ...
        expect(await adventureFractionUpdatePage.getNameInput()).to.eq('name');
        expect(await adventureFractionUpdatePage.getDescInput()).to.eq('desc');
        expect(await adventureFractionUpdatePage.getImageInput()).to.endsWith(fileNameToUpload);
        await adventureFractionUpdatePage.save();
        expect(await adventureFractionUpdatePage.getSaveButton().isPresent()).to.be.false;

        expect(await adventureFractionComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1);
    });
