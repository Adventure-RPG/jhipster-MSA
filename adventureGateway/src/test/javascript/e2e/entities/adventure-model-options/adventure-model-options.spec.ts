...
describe('AdventureModelOptions e2e test', () => {
    ...
    it('should load create AdventureModelOptions page', async () => {
        ...
        expect(await adventureModelOptionsUpdatePage.getPageTitle()).to.eq(
            'adventureGatewayApp.adventureModelOptions.home.createOrEditLabel'
        );
    ...
    it('should create and save AdventureModelOptions', async () => {
        ...
        expect(await adventureModelOptionsUpdatePage.getColorInput()).to.eq('color');
        await adventureModelOptionsUpdatePage.save();
        expect(await adventureModelOptionsUpdatePage.getSaveButton().isPresent()).to.be.false;

        expect(await adventureModelOptionsComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1);
    });
