...
describe('AdventureScript e2e test', () => {
    ...
    it('should create and save AdventureScripts', async () => {
        ...
        expect(await adventureScriptUpdatePage.getNameInput()).to.eq('name');
        expect(await adventureScriptUpdatePage.getFileInput()).to.endsWith(fileNameToUpload);
        expect(await adventureScriptUpdatePage.getArgumentsScriptInput()).to.eq('argumentsScript');
        await adventureScriptUpdatePage.save();
        expect(await adventureScriptUpdatePage.getSaveButton().isPresent()).to.be.false;

        expect(await adventureScriptComponentsPage.countDeleteButtons()).to.eq(nbButtonsBeforeCreate + 1);
    });
